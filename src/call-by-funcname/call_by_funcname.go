...
/*
最主要的是根据函数所在的结构体对象根据函数名称获取函数值，即通过reflect.TypeOf(calc).MethodByName(funcName) 来获取函数的值，然后再使用[]reflect.Value来组装函数调用所需要的参数，其中最重要的是params[0]必须是函数所在结构体对象的值，其他的则为对应函数的参数了。最后使用fn.Func.Call方法调用函数即可。
*/
