...
func main() {
	...
	client, err := elastic.NewClient(elastic.SetURL("http://10.0.6.22:9200"), elastic.SetBasicAuth("elastic", "changeme"))
	if err != nil {
		// Handle error
		panic(err)
	}
	...
	info, code, err := client.Ping("http://10.0.6.22:9200").Do(ctx)
	if err != nil {
		// Handle error
		panic(err)
	}
	...
	esversion, err := client.ElasticsearchVersion("http://10.0.6.22:9200")
	if err != nil {
		// Handle error
		panic(err)
	}
